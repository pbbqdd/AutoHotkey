...
Loop
{
    ...
    if (current_axisV_position != prev_axisV_position || current_axisU_position != prev_axisU_position)
    {
        ...
        ; 例如：
             if ( axis_temp > 40 and current_axisV_position <= l_act && current_axisU_position >axisU_th ) {
           Send, {LWin down}{1}{LWin up} ; 如果摇杆向左移动超过阈值，发送左箭头键
           axis_temp=40
         } else if (current_axisV_position >= r_act and axis_temp < 60 && current_axisU_position >axisU_th) {
            Send, {LWin down}{2}{LWin up} ; 如果摇杆向右移动超过阈值，发送右箭头键
             axis_temp=60
         } else if (current_axisV_position > 47 && current_axisV_position < 53 && axis_temp !=50 && current_axisU_position >axisU_th){
